...
Current evidence score: **76/100** after the public 0.4.12 deployment,
...
| Criterion | Points | Current evidence |
|---|---:|---|
| Versioned GitHub release and one-command Homebrew install | 4/4 | 0.4.12 public asset hash plus isolated install verified; CI run 30802423567 proves the Node 22 Homebrew formula installs, executes 0.4.12, tests, uninstalls, and leaves both CLI links absent on clean macOS 26 and Linux |
| Host setup installs and verifies a reachable matching service | 4/4 | Matching-version 0.4.12 doctor receipts on the live Reagan and Hudson hosts; one session-scoped invite command now atomically creates the invite and ready room |
| Fresh teammate can paste an invite without prior setup | 4/4 | Child-process regression and live Hudson → Reagan receipt establish identity, store the credential outside config, and enter the exact shared room from the printed join command |
...
| Criterion | Points | Current evidence |
|---|---:|---|
| Automated behavioral and syntax suite | 4/4 | 46/46 tests and syntax checks; CodeQL extended scan runs on pushes, pull requests, and weekly with zero open alerts after its bearer-parser finding was fixed |
| Real service restart and matching-version health | 2/2 | Live Reagan and Hudson 0.4.12 public-asset installs, service restarts, matching doctor receipts, reciprocal session reads, and Hudson's managed-provider restart with post-turn health |
...
1. **Closed:** Homebrew 0.4.12 installs, executes, tests, and uninstalls on clean
