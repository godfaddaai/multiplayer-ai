...
| Vibeyard | An MIT-licensed agent IDE can attract meaningful open-source interest with WebRTC terminal sharing, read-only/read-write roles, PIN authentication, and support for Claude Code, Codex, and Gemini. Its repository had 1,331 stars on August 2, 2026. | It asks the team to adopt a new IDE and shares terminal I/O. `mpai` must not compete as generic P2P terminal sharing; its wedge is joining separately named sessions that already exist in native tools, with explicit session visibility and attributed/audited human turns. [Vibeyard](https://github.com/elirantutia/vibeyard) |
